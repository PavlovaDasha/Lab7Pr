...
public class BandList {
     ...
     */
    private static List<MusicBand> musicBandList = Collections.synchronizedList(new ArrayList<>());


    public static List<MusicBand> getMusicBandList() {
     ...
     * @param id идентификационный номер
     */
    public String removeById(int id,String user) {
        try {
            MusicBand band = musicBandList.stream().filter(x->x.getId()==id).findFirst().get();
            if (band.getUser().equals(user)) {
                musicBandList.remove(band);
                return "Элемент c id=" + id + " удалён";
     ...
     */
    public String removeAt(int index,String user) {
        if (index < musicBandList.size()) {
            if (musicBandList.get(index).getUser().equals(user)) {musicBandList.remove(index); return "Элемент на месте"+index+" удалён";}
            else return "Элемент не принадлежит вам.";
        }else {
            return "no such band";
        }
